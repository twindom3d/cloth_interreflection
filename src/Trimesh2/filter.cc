...
//  Proc. SIGGRAPH, 2004.
inline void inflate(TriMesh *mesh, float amount)
{
	mesh->need_normals();
...
inline void apply_xform(TriMesh *mesh, const xform &xf)
{
	int nv = mesh->vertices.size();
	for (int i = 0; i < nv; i++)
...
// Translate the mesh
inline void trans(TriMesh *mesh, const vec &transvec)
{
...
inline void rot(TriMesh *mesh, float r, const vec &axis)
{
...
inline void scale(TriMesh *mesh, float s)
{
...
inline void scale(TriMesh *mesh, float sx, float sy, float sz)
{
...
inline void scale(TriMesh *mesh, float s, const vec &d)
{
...
inline void clip(TriMesh *mesh, const TriMesh::BBox &b)
{
	int nv = mesh->vertices.size();
...
inline point point_center_of_mass(const vector<point> &pts)
{
...
inline point mesh_center_of_mass(TriMesh *mesh)
{
...
inline void point_covariance(const vector<point> &pts, float C[3][3])
{
	for (int j = 0; j < 3; j++)
		for (int k = 0; k < 3; k++)
			C[j][k] = 0.0f;
...
inline void mesh_covariance(TriMesh *mesh, float C[3][3])
{
	if (mesh->faces.empty() && mesh->tstrips.empty()) {
...
inline void normalize_variance(TriMesh *mesh)
{
...
inline void pca_rotate(TriMesh *mesh)
{
	point com = mesh_center_of_mass(mesh);
	trans(mesh, -com);
...
inline void pca_snap(TriMesh *mesh)
{
	point com = mesh_center_of_mass(mesh);
	trans(mesh, -com);
...
inline static float max_x(const TriMesh *mesh, int i)
{
...
inline void orient(TriMesh *mesh)
{
	mesh->need_faces();
	mesh->tstrips.clear();
...
inline void erode(TriMesh *mesh)
{
	int nv = mesh->vertices.size();
	vector<bool> bdy(nv);
	for (int i = 0; i < nv; i++)
...
inline void noisify(TriMesh *mesh, float amount)
{
	mesh->need_normals();
	mesh->need_neighbors();
	int nv = mesh->vertices.size();
