...
#pragma omp parallel for
	...
	for (int i = 0; i < nf; i++) {
		Face &v = mesh->faces[i];
		Face &n = newverts[i];
		mesh->faces.push_back(Face(v[0], n[2], n[1]));
		mesh->faces.push_back(Face(v[1], n[0], n[2]));
		mesh->faces.push_back(Face(v[2], n[1], n[0]));
		v = n;
	}
